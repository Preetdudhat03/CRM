...
async function handleDealWon(data) {
    ...
    // 2. Insert into notifications table (SINGLE broadcast record)
    // The Edge Function will pick this up and send FCM to all users with matching roles.
    const insertQuery = `
        INSERT INTO notifications (title, message, related_entity_type, related_entity_id)
        VALUES ($1, $2, $3, $4)
    `;
    ...
    try {
        await db.query(insertQuery, [
            ...
            `deal_won||roles:Super Admin,Admin,Manager,super_admin,admin,manager,agent,viewer||sender:${performerId || ''}`,
            dealId
        ]);
        console.log(`[Notification Worker] Broadcast notification inserted for deal ${dealId}`);
    } catch (e) {
        console.error(`[Notification Worker] Failed to insert broadcast notification`, e);
    }
}
